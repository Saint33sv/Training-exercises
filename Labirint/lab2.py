...
# Вернуть соседа непосредственно сверху, снизу, справа и слева
def get_neighbors(mat, r, c):
    shape = mat.shape
    neighbors = []
    # убедитесь, что соседи находятся в пределах границ изображения
...
def bubble_down(queue, index):
    ...
    if lc_index >= length:
        return queue
    if lc_index < length and rc_index >= length:  # только что покинутый ребенок
...
def find_shortest_path(img, src, dst):
    pq = []  # очередь с минимальным приоритетом кучи
    source_x = src[0]
    source_y = src[1]
    dest_x = dst[0]
    dest_y = dst[1]
...
p = find_shortest_path(img, (24, 102), (977, 896))
drawPath(img, p)
